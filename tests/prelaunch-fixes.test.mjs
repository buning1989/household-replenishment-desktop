...
// 注意：实际签名是 isEmptyInitialCandidate(state, fromFallback)。
//   fromFallback === true 表示本次 loadState 走了 fallback（localStorage 异常/为空）。
//   仅当 fromFallback && items.length === 0 才视为空初始态。
//   没有 onboarding.completed 字段（AppState 已移除该字段）。
function isEmptyInitialCandidate(state, fromFallback = false) {
  return fromFallback && state.items.length === 0
}

// 模拟 reconcileState：localFromFallback 模拟 store.ts 里的 lastLoadUsedFallback，
// remote 永远视作主进程返回的数据，按当前实现约定走 fromFallback=true 判定。
function reconcileStateLogic(localState, remoteRaw, localFromFallback = false) {
  if (!remoteRaw) {
    return localState
  }
  // 模拟 migrateState（简化：假设 remoteRaw 已是合法 AppState）
  const remoteState = remoteRaw
  const localEmpty = isEmptyInitialCandidate(localState, localFromFallback)
...
test("P1-1: reconcileState 逻辑 - localStorage 为空、主进程有效：恢复主进程数据", () => {
  ...
  // localFromFallback=true 表示 localStorage 异常/为空，触发恢复主进程
  const result = reconcileStateLogic(localEmpty, remoteValid, true)
  assert.equal(result.items.length, 1, "应恢复主进程的 1 个物品")
  assert.equal(result.items[0].name, "卫生纸")
  assert.equal(result, remoteValid, "应整体返回主进程的版本")
})
...
test("P1-1: reconcileState 逻辑 - 两边都为空初始状态：选择较新的", () => {
  ...
  // localFromFallback=true：两边都视作空初始态，按 updatedAt 选较新的（remote）
  const result = reconcileStateLogic(localEmpty, remoteEmpty, true)
  assert.equal(result.updatedAt, now, "应选择较新的版本")
})

test("P1-1: reconcileState 逻辑 - local 空但未走 fallback：不视作空初始态，按 updatedAt 比较", () => {
  // 用户主动清空 items 后（非 fallback），即使 items 为空，也不应被当作"空初始态"恢复主进程
  const now = Date.now()
  const localClearedByUser = {
    version: 3,
    categories: ["其他用品"],
    items: [],
    settings: { reminderIntervalMinutes: 60, quietStart: "22:00", quietEnd: "08:00", snoozeUntilHour: 8 },
  ...
  const remoteValid = {
    version: 3,
    categories: ["卫生间"],
    items: [{ id: "remote-1", name: "卫生纸", category: "卫生间", type: "learning", cycleDays: 30, bufferDays: 5, lastRestockedAt: now, anchorEstimated: true, purchaseOptions: [], history: [], createdAt: now, updatedAt: now, learningEnabled: true }],
    settings: { reminderIntervalMinutes: 60, quietStart: "22:00", quietEnd: "08:00", snoozeUntilHour: 8 },
    householdProfile: null,
    updatedAt: now - 100000
  }

  // localFromFallback=false（默认）：local 不视作空初始态，按 updatedAt 比较，local 更新 → 保留 local
  const result = reconcileStateLogic(localClearedByUser, remoteValid)
  assert.equal(result, localClearedByUser, "非 fallback 的空状态不应被恢复覆盖")
  assert.equal(result.items.length, 0, "应保留用户主动清空后的空状态")
})
