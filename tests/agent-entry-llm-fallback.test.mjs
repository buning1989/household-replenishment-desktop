...
// ---------- 1. pendingCollection + 拼夕夕 → 本地解析 platform=拼多多 ----------

test("1. pendingCollection + 拼夕夕 → 本地解析 platform=拼多多", () => {
  const state = makeState({ items: [] })
  const orch = createHouseholdOrchestrator()
  const pendingCollection = buildWipesCollection()

  // 拼夕夕 现由本地 parsePlatform 处理 → sync, 不调用 LLM
  const d = decide(orch, {
    text: "拼夕夕",
    state,
    itemViews: [],
...
// ---------- 2. pendingCollection + pdd → 本地解析 platform=拼多多 ----------

test("2. pendingCollection + pdd → 本地解析 platform=拼多多", () => {
  const state = makeState({ items: [] })
  const orch = createHouseholdOrchestrator()
  const pendingCollection = buildWipesCollection()

  // pdd 现由本地 parsePlatform 处理 → sync, 不调用 LLM
...
// ---------- 4. pendingCollection + 多多 → 本地解析 platform=拼多多 ----------

test("4. pendingCollection + 多多 → 本地解析 platform=拼多多", () => {
  const state = makeState({ items: [] })
  const orch = createHouseholdOrchestrator()
  const pendingCollection = buildWipesCollection()

  // 多多 现由本地 parsePlatform 处理 → sync, 不调用 LLM
...
test("11. pendingCollection + p'd'd + LLM 调用失败 → clarification", async () => {
  const state = makeState({ items: [] })
  const orch = createHouseholdOrchestrator()
  const pendingCollection = buildWipesCollection()

  // p'd'd 仍需 LLM（撇号破坏别名匹配），LLM 调用失败 → clarification
  const d = decide(orch, { text: "p'd'd", state, itemViews: [], pendingCollection })
  assert.equal(d.kind, "needTurnInterpreterLlm")
