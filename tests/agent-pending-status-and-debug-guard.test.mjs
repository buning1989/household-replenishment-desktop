...
// ---------- 9. 纯文本 answer fallback 防线：包含禁用词时不放宽 ----------

test("9. normalizeLlm 纯文本 answer：包含「超出家务范围」时不放宽，返回 null 走原 fallback", () => {
  const orch = createHouseholdOrchestrator()
  const state = makeState({ items: [] })
  ...
  // 包含禁用词，不应走纯文本 answer fallback，返回 null 让 App.tsx 走原 fallback
  assert.equal(turn, null, "包含禁用词时应返回 null")
})

// ---------- 10. 纯文本 answer fallback 防线：JSON 结构不当作纯文本 ----------

test("10. normalizeLlm 纯文本 answer：包含 JSON 结构时不当作纯文本 answer", () => {
  const orch = createHouseholdOrchestrator()
  const state = makeState({ items: [] })

  // 不合法的 JSON（parse 失败），但包含 { } 结构，不应走纯文本 fallback
  ...
  // 含 JSON 结构，不放宽为纯文本 answer，返回 null
  assert.equal(turn, null, "含 JSON 结构时不应放宽为纯文本 answer")
})
