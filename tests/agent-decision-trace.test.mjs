...
test("11. 45块 → 本地高置信 price=45, trace.llmInterpreter 不应被填充", () => {
  ...
  // 阶段 3B.1：createTrace 默认初始化 llmInterpreter（called=false）
  // 本地高置信路径（45块）不进入 LLM
...
test("28. formatTraceForCopy 未填充字段显示 not captured / no LLM call", () => {
  ...
  // 阶段 3B.1：createTrace 默认初始化 llmInterpreter（called=false, skipReason=local_high_confidence），
  // formatTraceForCopy 走 llmInterpreter 分支，显示 (turnInterpreter) shouldCall=false, called=false
  assert.ok(text.includes("called=false"), "llmInterpreter.called=false 应显示")
  assert.ok(text.includes("skipReason=local_high_confidence"), "skipReason 应显示")
  assert.ok(text.includes("no LLM response") || text.includes("llmInterpreter.called=false"), "llmResponse 未填充应显示 no LLM response 或 called=false")
})
