...
test("10. pendingCollection=猫砂 + 「就这样」→ planCommand/draftCommit（强制保存）", () => {
  const state = makeState({ items: [makeItem("i1", "猫砂", "宠物用品")] })
  const orch = createHouseholdOrchestrator()
  const pendingCollection = buildCatSandCollection()
  ...
  assert.equal(d.kind, "sync")
  assert.equal(d.turn.kind, "planCommand", "强制保存应转 planCommand/draftCommit")
...
test("12. pendingCollection=宠物擦脚湿巾 + 「按这个来」→ planCommand/draftCommit（与「就这样」一致）", () => {
  const state = makeState({ items: [] })
  const orch = createHouseholdOrchestrator()
  ...
  // 在 pendingCollection 上下文中触发 force_proposal → 强制转 planCommand/draftCommit
  assert.equal(d.kind, "sync", "应返回 sync，不应进入 route_to_llm")
  assert.equal(d.turn.kind, "planCommand", "应转 planCommand/draftCommit，与「就这样」一致")
  assert.equal(d.turn.command.command, "draftCommit")
