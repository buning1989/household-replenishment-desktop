...
test("buildPostCommitObservation: 高价补货命中 priceAnomaly", () => {
  const now = Date.now()
  // 历史均价 100，本次 130（新口径：avg 排除最新 = 100，pct=30%，超 10% 阈值）
  const item = makeItem({
    history: [
...
test("验收: 录入单价高于均价 15% 的补货并确认，结果消息含价格提示", () => {
  ...
  const state = makeState({ items: [item] })
  // 本次补货单价 115（高于均价 15%；新口径 avg=100，pct=15%，超 10% 阈值）
  const draft = {
    kind: "restock",
    itemId: "i1",
    itemName: "猫砂",
  ...
  assert.match(result.observation, /猫砂/)
  assert.match(result.observation, /贵了/)
  assert.match(result.observation, /15%/)
  // summary 仍正常
