...
test("orchestrator: 「你是谁」→ answer turn，短句管家口吻", () => {
  const state = makeState()
  const orch = createHouseholdOrchestrator()
  const decision = orch.decide({
    text: "你是谁",
  ...
  assert.equal(decision.kind, "sync")
  assert.equal(decision.turn.kind, "answer")
  assert.ok(decision.turn.message.includes("403 家庭管家"))
  const sentences = decision.turn.message.split(/[。？！]/).filter((s) => s.trim().length > 0)
  assert.ok(sentences.length <= 2, `应不超过两句话，实际 ${sentences.length} 句`)
})

test("orchestrator: 「这周可能要补什么」→ answer turn，区分 overdue/upcoming", () => {
  ...
  assert.equal(decision.kind, "sync")
  assert.equal(decision.turn.kind, "answer")
  assert.ok(decision.turn.message.includes("先处理已经到点的"))
  assert.ok(decision.turn.message.includes("接下来 7 天"))
  assert.ok(!decision.turn.message.includes("未来 7 天有"))
})

// ---------- 4. needLlm 场景 ----------

test("orchestrator: 本地无法处理的查询 → needLlm", () => {
  const state = makeState()
  const orch = createHouseholdOrchestrator()
  const decision = orch.decide({
    text: "最近天气怎么样",  // 非家庭消耗品问题，本地无法处理
    state,
    itemViews: [],
    dateContext: buildChatDateContext(Date.UTC(2026, 6, 4))
  })
