...
test("B3: 首次产出 restock 草稿缺金额时，collection message 含参考文案（不追问「多少钱」）", () => {
...
test("B3: revise 路径不重复采集（revise 后仍缺金额也不追加）", () => {
  ...
  assert.ok(d1.kind === "sync" && d1.turn.kind === "collection")
  // 从 collection 提取 draft，模拟转 proposal 后用户修订数量
  const pendingDraft = d1.turn.collection.draft
...
test("B3: confirm 路径不追加参考", () => {
  ...
  // 首次产出 collection，从中提取 draft 模拟已转 proposal 后的 confirm
  const pendingDraft = d1.turn.collection.draft

  // 用户确认：orchestrator 返回 planCommand/draftCommit，外层 App.tsx 会执行 commit
  const d2 = orch.decide({
    text: "确认吧",
    state,
    itemViews: [],
  ...
  assert.ok(d2.kind === "sync")
  assert.ok(d2.turn.kind === "planCommand")
...
test("B4 验收: 缺金额草稿 → 给参考 → 回「45块」→ 价格补进 → 不再追加参考", () => {
  ...
  // 2. 用户回「45块」：应命中 collection supplement，价格补进
  const d2 = orch.decide({ text: "45块", state, itemViews: [], pendingCollection, dateContext })
  assert.ok(d2.kind === "sync")
  // 价格补进后 completeness = readyToConfirm，自动转 proposal
  assert.ok(d2.turn.kind === "proposal" || d2.turn.kind === "collection", `期望 proposal 或 collection, 实际: ${d2.turn.kind}`)
  if (d2.turn.kind === "proposal") {
    assert.equal(d2.turn.executableDraft.price, 45, "「45块」应补进 price")
    assert.ok(!d2.turn.message.includes("多少钱"), "补进后不应追问")
    assert.ok(!d2.turn.message.includes("我先按"), "补进后不应再追加参考")
  } else {
    // 如果仍是 collection，检查价格已补进
    assert.equal(d2.turn.collection.draft.price, 45, "「45块」应补进 price")
  }
})
...
test("B4 验收: 缺金额草稿 → 给参考 → 回纯数字「45」→ 价格补进 → 不再追加参考", () => {
  ...
  const d1 = orch.decide({ text: "帮我加一袋猫砂", state, itemViews: [], dateContext })
  const pendingCollection = d1.turn.collection
  assert.equal(pendingCollection.draft.price, undefined)
  assert.ok(!d1.turn.message.includes("多少钱"))

  // 用户回纯数字「45」（无单位）：也应补进 price
  const d2 = orch.decide({ text: "45", state, itemViews: [], pendingCollection, dateContext })
  assert.ok(d2.kind === "sync")
  assert.ok(d2.turn.kind === "proposal" || d2.turn.kind === "collection")
  if (d2.turn.kind === "proposal") {
    assert.equal(d2.turn.executableDraft.price, 45, "纯数字「45」应补进 price")
  } else {
    assert.equal(d2.turn.collection.draft.price, 45, "纯数字「45」应补进 price")
  }
...
test("B4 验收: 缺金额草稿 → 给参考 → 回「确认」→ 正常写入 → 不再追加参考", () => {
  ...
  // 采集态下用户说「就这样」：应触发 forceProposal，转 planCommand/draftCommit
  const d2 = orch.decide({ text: "就这样", state, itemViews: [], pendingCollection, dateContext })
  assert.ok(d2.kind === "sync")
  assert.ok(d2.turn.kind === "planCommand", `「就这样」应触发 forceProposal 转 planCommand/draftCommit, 实际: ${d2.turn.kind}`)
  assert.equal(d2.turn.command.command, "draftCommit")
  assert.ok(!d2.turn.message.includes("多少钱"), "forceProposal 路径不应追问")
  assert.ok(!d2.turn.message.includes("我先按"), "forceProposal 路径不应追加参考")
})
