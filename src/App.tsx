...
function App() {
  ...
  function handleAgentDraftConfirm(agentDraft: AgentDraft): { summary: string; links: AgentMessageLink[]; observation?: string; insight?: string } {
    const dateContext = buildChatDateContext()
  ...
  function handleAgentDraftBatchConfirm(drafts: AgentDraft[]): { summary: string; links: AgentMessageLink[]; observation?: string; insight?: string } {
  ...
  function handleAgentPlanConfirm(plan: AgentPlan): { summary: string; links: AgentMessageLink[]; observation?: string; insight?: string } {
    const dateContext = buildChatDateContext()
...
function HouseholdChatPanel({ state, itemViews, messages, onMessagesChange, onQuestionSent, onConfirmDraft, onConfirmBatch, onConfirmPlan, onUndoLastMutation, onCorrectLastMutation, onOpenItem, onOpenCategory, onOpenSettings, orderImageApiKey, orderImageModel, orderRecognitionMode, seenObservationKeys, scrollMemoryRef }: {
  ...
  function confirmBatch(messageIndex: number, baseMessages: HouseholdChatMessage[]) {
    ...
    // 拼接 summary + observation + insight
    const parts = [result.summary]
    if (result.observation) parts.push(result.observation)
    if (result.insight) parts.push(result.insight)
    const content = parts.join(" ")
  ...
  useEffect(() => {
    ...
    if (decision.kind === "sync") {
      ...
      // 阶段 3B：若旧 pendingDraft 存在，也标 superseded
      // 阶段 3C：若旧 pendingBatch 存在，也标 superseded
      if (turn.kind === "collection") {
        ...
        // 阶段 3C：新 collection supersede 旧 pendingBatch，避免下一轮 batch handler 抢焦点
        if (pendingBatchMessageIndex >= 0) {
          base = supersedeOldPendingBatch(base)
        }
